...
namespace NiconicomeTest.Stabs.Models.Domain.Niconico.NicoHttpStabs
{
    class NicoHttpStab : INicoHttp
    {
        ...
        public Task<HttpResponseMessage> SendAsync(HttpRequestMessage m)
        {
            var msg = new HttpResponseMessage
            {
                StatusCode = HttpStatusCode.OK,
                ReasonPhrase = "Generated by stab",
                Content = new StringContent($"HTTP {m.Method.Method} Method")
            };

            return Task.FromResult(msg);
        }

        public Task<HttpResponseMessage> SendAsync(HttpRequestMessage m, HttpCompletionOption optn = HttpCompletionOption.ResponseContentRead)
        {
            var msg = new HttpResponseMessage
            {
                StatusCode = HttpStatusCode.OK,
