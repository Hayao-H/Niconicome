...
namespace Niconicome.Models.Domain.Niconico.Download.Comment.V2.Core.V3
{
    public interface ICommentCollection
    {
        ...
        /// </summary>
        /// <returns></returns>
        IReadOnlyList<UnFilledRange> GetUnFilledRange(string thread, string fork, int originationNo = 0);
    ...
    {
        public CommentCollection(int commentCount)
        {
        ...
        public IReadOnlyList<UnFilledRange> GetUnFilledRange(string thread, string fork, int originationNo = 0)
        {
            var list = new List<UnFilledRange>();

            var collection = this.GetCollection(thread, fork);
            IReadOnlyList<(IComment, int)> ranges = collection.GetUnFilledRange(originationNo);
