...
namespace Niconicome.Models.Domain.Local.Addons.Core.Installer
{
    ...
    public class AddonUninstaller : IAddonUninstaller
    {
        ...
        public IAttemptResult Uninstall(int id)
        {
            ...
            try
            {
                this.fileIO.AppendText(Path.Combine(FileFolder.AddonsFolder, Const::AdddonConstant.UninstalledAddonsFile), addon.PackageID.Value);
            }
            catch (Exception ex)
            {
        ...
        {
            string delAddonsList = Path.Combine(FileFolder.AddonsFolder, Const::AdddonConstant.UninstalledAddonsFile);
